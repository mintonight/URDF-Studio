...
> 最后更新：2026-07-05 | 技术栈：React 19.2 + TypeScript 5.8 + Three.js/R3F + Vite 6.2 + Tailwind CSS 4.1 + Zustand 5
...
│   ├── hooks/      跨 store / viewer / source 同步 hook（file-export/workspace-mutations/workspace-source-sync）
...
- workspace/source 同步策略优先放 `src/app/hooks/workspace-source-sync/` 或 `workspace-mutations/` 子模块，不继续膨胀 `useWorkspaceSourceSync.ts`
...
单元测试继续邻近源码放置：`src/**/*.test.*` / `src/**/*.spec.*`。当前 runner 管理 `src` 邻近测试 593 个、全量 Node 测试 599 个。不要把普通单元测试搬到统一 `tests/` 目录。
...
`npm test` 保持 fast lane，避免默认验证依赖 `test/` 大型语料或浏览器环境。需要全量覆盖时显式使用 `npm run test:unit:all`、`npm run test:fixtures` 或 `npm run verify:full`。
