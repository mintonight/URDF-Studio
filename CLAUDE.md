...
> 最后更新：2026-07-09 | 技术栈：React 19.2 + TypeScript 5.8 + Three.js/R3F + Vite 6.2 + Tailwind CSS 4.1 + Zustand 5
...
`RobotData` 只允许存在于 component 或只读 projection；不得在 Zustand 顶层恢复第二份可写 `name/links/joints/components`。跨 store 协调优先放 `app/hooks/*`；USD 中间态优先落在 `assetsStore` 或 `app/utils/*`。
