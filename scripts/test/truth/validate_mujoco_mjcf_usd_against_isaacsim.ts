...
const DEFAULT_ARTIFACT_ROOT = path.resolve('tmp/regression/mujoco-mjcf-isaacsim');
const DEFAULT_OUTPUT_PATH = path.resolve('tmp/regression/mujoco-mjcf-isaacsim-summary.json');
...
function printUsage(): void {
  ...
  npx tsx scripts/test/truth/validate_mujoco_mjcf_usd_against_isaacsim.ts [options]

Options:
  --fixture-root <path>   MuJoCo fixture root. Default: ${DEFAULT_FIXTURE_ROOT}
...
async function discoverFixtures(options: Options): Promise<{
  ...
  for (const supportRootAbs of supportRoots) {
    ...
    for (const file of context.availableFiles) {
      if (!SOURCE_FORMATS.has(file.format)) {
        continue;
      }

      const stem = path.basename(file.name, path.extname(file.name));
      const id = createFixtureId(supportRootRel, file.name);
      const haystack = `${id} ${packageName} ${stem} ${file.name}`;
  ...
  if (fixtures.length === 0) {
    fail('No MuJoCo source fixtures matched the selected filters.');
  }
...
async function buildExtraMeshFiles(
  fixtureRoot: string,
...
function extractSuccessfulArticulationAttempt(summary: JsonRecord | null): JsonRecord | null {
  const preferredCandidates = new Set(
    (Array.isArray(summary?.articulation_root_candidates)
      ? summary.articulation_root_candidates
      : []
    )
  ...
  const attempts = (Array.isArray(summary?.attempts) ? summary.attempts : [])
    .map((attempt) => attempt as JsonRecord)
    .filter((attempt) => attempt.initialized === true);
  if (attempts.length === 0) {
    return null;
  }

  return [...attempts].sort((left, right) => {
    const leftPreferred = preferredCandidates.has(String(left.candidate || '').trim());
    const rightPreferred = preferredCandidates.has(String(right.candidate || '').trim());
    ...
    const leftBodyCount = Number.isFinite(Number(left.body_count)) ? Number(left.body_count) : -1;
    const rightBodyCount = Number.isFinite(Number(right.body_count)) ? Number(right.body_count) : -1;
    if (leftBodyCount !== rightBodyCount) {
      return rightBodyCount - leftBodyCount;
    }

    return String(left.candidate || '').localeCompare(String(right.candidate || ''));
  })[0]!;
}
...
function normalizePhysicsBodies(summary: JsonRecord): Record<string, PhysicsBodySummary> {
  ...
  return Object.fromEntries(
    Object.entries(rigidBodies)
      .map(([bodyPath, value]) => {
        const record = value as JsonRecord;
        const normalizedBodyPath = normalizeBodyIdentity(bodyPath) || bodyPath;
        return [
          normalizedBodyPath,
          {
...
function hasMeaningfulInertial(link: { inertial?: unknown }): boolean {
  if (!link.inertial || typeof link.inertial !== 'object') {
    return false;
  }

  const inertial = link.inertial as {
    mass?: unknown;
    origin?: {
      xyz?: {
        x?: unknown;
  ...
  if (!Number.isFinite(mass) || Math.abs(mass) <= 1e-12) {
    return false;
  }
...
function extractExpectedCollisionBodyCounts(robot: {
  ...
  const linkNames = new Set(
    Object.values(robot.links)
      .map((link) => String(link.name || link.id || '').trim())
      .filter((linkName) => linkName.length > 0),
  );
  ...
  Object.values(robot.links).forEach((link) => {
    ...
    if (expectedCollisionCount <= 0) {
      return;
    }

    const linkName = String(link.name || link.id || '').trim();
    const syntheticMatch = MJCF_SYNTHETIC_GEOM_LINK_RE.exec(linkName);
    const normalizedLinkName =
      syntheticMatch && linkNames.has(syntheticMatch[1] || '')
        ? (syntheticMatch[1] as string)
        : linkName;
    const bodyPath = `/${normalizedLinkName}`;
    counts.set(bodyPath, (counts.get(bodyPath) || 0) + expectedCollisionCount);
  });
...
function extractExplicitInertialBodies(robot: {
  ...
  return Object.values(robot.links)
    .filter((link) => hasMeaningfulInertial(link))
...
function evaluateCurrentExportAgainstTruth(params: {
  ...
  if (currentAttempt && truthAttempt) {
    const normalizedCurrentCandidate = normalizeBodyIdentity(currentAttempt.candidate);
    const normalizedTruthCandidate = normalizeBodyIdentity(truthAttempt.candidate);
    if (normalizedCurrentCandidate !== normalizedTruthCandidate) {
      softFailures.push(
