...
| **结构树 CRUD**       | `test:browser:tree-crud`                   | addChild, deleteSubtree, deleteLink, renameLink, renameRobot, setLinkVisibility, setAllLinksVisibility, undo/redo (多轮)                             |
| **属性编辑**          | `test:browser:property-editor`             | joint origin/axis/limit/dynamics/friction/type, link visualCount/collisionCount/inertial, joint angle (单个+批量), property panel                    |
| **碰撞/显示**         | `test:browser:viewer`                      | showCollision, showJointAxes, showOrigins, showCenterOfMass, showInertia, tool mode 切换, runtime transforms 联动, canvas                            |
...
| **源码编辑器**        | `test:browser:source-editor`               | Monaco 编辑, auto-apply, XML 修改, re-import 恢复                                                                                                    |
| **组装拼接**          | `test:browser:assembly`                    | initAssembly, addComponent×2, updateComponentTransform, toggleComponentVisibility, addBridge, updateBridge, removeBridge, removeComponent, undo/redo |
| **导出验证**          | `test:browser:export`                      | URDF XML 生成验证 (link/joint/parent/child 一致), 重名检测, UI 导出选项, 导出后状态完整性                                                            |
...
```bash
npm run test:setup && npm run test:browser:all
```
...
│   └── ... (现有回归脚本)
├── benchmark/ truth/ fixtures/ runner/
└── README.md
```
