...
test('plane and full cylinder are enabled for analytic face construction', () => {
...
test('ENABLED_STEP_ANALYTIC_SURFACES contains plane and cylinder', () => {
  assert.equal(ENABLED_STEP_ANALYTIC_SURFACES.size, 2);
  assert.equal(ENABLED_STEP_ANALYTIC_SURFACES.has('plane'), true);
  assert.equal(ENABLED_STEP_ANALYTIC_SURFACES.has('cylinder'), true);
});
